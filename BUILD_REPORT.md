...
- RNAMOD vs IAMOD: `INSUFFICIENT_INFORMATION` — RNAMOD variant, corruption, preprocessing, ratios, repetitions, and metric remain unavailable.
- RNAMOD vs SCoNE: `INSUFFICIENT_INFORMATION` — complete-view/sample context matches, but RNAMOD's experimental fingerprint is missing.
- RNAMOD vs NCMOD: `INSUFFICIENT_INFORMATION` — insufficient RNAMOD primary evidence.
- RNAMOD vs MODGD: `INSUFFICIENT_INFORMATION` — insufficient RNAMOD primary evidence.
...
- NCMOD vs SCoNE: `PARTIALLY_COMPARABLE` where normalized to a matched setting; captured variants, operators/aggregation, and repetitions differ.

Pair counts over all 11 priority records are generated by the final audit command rather than presented as a leaderboard; an `INSUFFICIENT_INFORMATION` result is a scientific finding, not a validation failure.
